<x-modal id="request-modal" maxWidth="2xl">
    <form class="flex flex-col items-center w-full gap-2" id="requestForm" action="{{ route('production.fertil.storeMaterialRequest') }}" method="POST">
        @csrf

        <x-wrapper-form-1>
            <x-wrapper-form-2>
                <x-tittle-form id="request-modal-title">Solicitar Materiales a Almacén</x-tittle-form>
                <p class="text-sm text-gray-500">Crear una nueva solicitud de productos para el área de Fertil</p>
            </x-wrapper-form-2>
        </x-wrapper-form-1>
