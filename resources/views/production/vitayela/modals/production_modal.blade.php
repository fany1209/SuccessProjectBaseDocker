<x-modal id="production-modal" maxWidth="lg">
    <form class="flex flex-col items-center w-full gap-2" id="production-form" action="{{ route('production.vitayela.storeProduction') }}" method="POST">
        @csrf
        ...
        <x-wrapper-form-1>
            <x-wrapper-form-2>
                ...
                <p class="text-sm text-gray-500">Registro de producción de Vitayela</p>
            </x-wrapper-form-2>
        </x-wrapper-form-1>
...
<script>
    function openCreateProduction() {
        document.getElementById('production-form').action = "{{ route('production.vitayela.storeProduction') }}";
    ...
    function editProduction(p) {
        document.getElementById('production-form').action = `/production/vitayela/productions/${p.vitayela_production_id}`;
