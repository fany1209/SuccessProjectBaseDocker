...
<div class="max-w-8xl mx-auto py-6 sm:px-6 lg:px-8">
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-bold text-gray-800">Producción de Proteamin</h2>
    ...
    <div class="mb-4 border-b border-gray-200">
        <ul class="flex flex-wrap -mb-px text-sm font-medium text-center" id="proteaminTab" role="tablist">
            ...
            <div class="overflow-x-auto">
                <table id="production-table" class="display w-full text-sm text-left text-gray-500">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>{{ $p->proteamin_production_id }}</td>
                            ...
                            <td class="text-right flex justify-end gap-2">
                                ...
                                <form action="{{ route('production.proteamin.destroyProduction', $p->proteamin_production_id) }}" method="POST" class="m-0 flex" onsubmit="confirmDelete(event, this, 'registro');">
                                    @csrf
                                    @method('DELETE')
            ...
            <div class="overflow-x-auto">
                <table id="inventory-table" class="display w-full text-sm text-left text-gray-500">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>{{ $inv->proteamin_inventory_id }}</td>
                            ...
                            <td class="text-right flex justify-end gap-2">
                                ...
                                <button type="button" onclick="openHistoryModal({{ $inv->proteamin_inventory_id }}, '{{ $inv->producto_descripcion }}')" class="text-white bg-yellow-500 hover:bg-yellow-600 rounded-sm w-8 h-8 flex items-center justify-center" title="Ver Historial">
                                ...
                                <form action="{{ route('production.proteamin.destroyInventory', $inv->proteamin_inventory_id) }}" method="POST" class="m-0 flex" onsubmit="confirmDelete(event, this, 'producto');">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="text-white bg-red-500 hover:bg-red-600 rounded-sm w-8 h-8 flex items-center justify-center" title="Eliminar">
                                        <svg class="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
...
@include('production.proteamin.modals.production_modal')
@include('production.proteamin.modals.inventory_modal')
@include('production.proteamin.modals.output_modal')
@include('production.proteamin.modals.history_modal')
