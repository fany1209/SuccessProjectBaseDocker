  ...
  <section class="col-span-12 w-full flex flex-col items-center px-1">
    ...
    <div class="w-full max-w-2xl mt-8">
      <div class="grid gap-4 sm:grid-cols-2">
        ...
        <a href="{{ route('cuentas-por-pagar.dashboard') }}" 
        ...
        <a href="{{ route('cuentas-por-pagar.facturas') }}" 
          class="inline-flex items-center {{ $btnBase }} {{ $btnCard }}">
            <img src="{{ asset('images/finance/dinero.png') }}" 
                alt="Facturas por Pagar" 
                class="{{ $imgCls }}">
            <span class="text-base font-semibold">Facturas por Pagar</span>
        </a>
      </div>
    </div>
...
<script>
  document.addEventListener('DOMContentLoaded', function() {
    @if(isset($pendingPaymentsCount) && $pendingPaymentsCount > 0)
      Swal.fire({
        title: '¡Atención!',
        text: 'Tienes {{ $pendingPaymentsCount }} factura(s) por pagar vencida(s). Por favor revisa el dashboard general.',
        icon: 'warning',
