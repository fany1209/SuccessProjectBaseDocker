    ...
    <form class="flex flex-col items-center w-full gap-2" id="add-factura-form">
        ...
        <x-wrapper-form-1>
            ...
            <x-wrapper-form-2>
                ...
                <select name="metodo_pago" class="w-full border-gray-300 rounded-md shadow-sm focus:border-green-500 focus:ring-green-500 text-sm py-2 px-3 bg-white">
        ...
        <x-wrapper-form-1>
            ...
            <x-wrapper-form-2>
                ...
                <x-input-1 name="pagador" placeholder="Nombre de la persona"></x-input-1>
            </x-wrapper-form-2>

            <div id="wrapper-terminacion" style="display: none;" class="w-full">
                <x-wrapper-form-2>
                    <x-label>Terminación (4 dígitos)</x-label>
                    <x-input-1 name="terminacion" maxlength="4" placeholder="1234"></x-input-1>
                </x-wrapper-form-2>
            </div>
        </x-wrapper-form-1>
...
$(function(){
   ...
   $('select[name="metodo_pago"]').on('change', function() {
    const valor = $(this).val();
    if (valor && valor.includes('Tarjeta')) {
        $('#wrapper-terminacion').fadeIn().find('input').prop('required', true);
    } else {
        $('#wrapper-terminacion').fadeOut().find('input').prop('required', false).val('');
    }
}).trigger('change');
